...
if DATABASE_URL:
    ...
    class _RowProxy(dict):
        """Dict that supports integer indexing like SQLite Row."""
    ...
    class PGCursor:
        """Mimics SQLite cursor — created fresh per execute call."""
...
def sql(q):
    """Translate SQLite query to PostgreSQL equivalents when DATABASE_URL is set."""
    if not DATABASE_URL:
        return q
    q = q.replace('?', '%s')
    ...
    def check_stale_services():
        ...
        while True:
            try:
                conn = get_db()
                if DATABASE_URL:
                    _sq = ("SELECT b.id, b.provider_id, b.client_name, b.service, b.client_phone, b.created_at,"
                ...
                stale = conn.execute(_sq).fetchall()
                for row in stale:
                    r = dict(row)
