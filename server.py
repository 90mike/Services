...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == '/api/bookings/accept':
            bid = body.get('booking_id')
            conn = get_db()
            brow = conn.execute("SELECT * FROM bookings WHERE id=?", (bid,)).fetchone()
            if brow:
                conn.execute("UPDATE bookings SET accepted=1 WHERE id=?", (bid,))
                conn.commit()
            conn.close()
            respond(self, {'success': True}); return
        ...
        if path == '/api/bookings/reject':
            bid = body.get('booking_id')
            conn = get_db()
            conn.execute("UPDATE bookings SET accepted=-1, status='rejected' WHERE id=?", (bid,))
            conn.commit(); conn.close()
            respond(self, {'success': True}); return
        ...
        if path == '/api/bookings':
            ...
            if phone:
                own = conn.execute("SELECT phone FROM providers WHERE id=?", (pid,)).fetchone()
            ...
            conn.execute("INSERT INTO bookings (id,provider_id,client_name,client_phone,service,date,notes,urgency) VALUES (?,?,?,?,?,?,?,?)",
                         (bid, pid, body.get('client_name'), phone,
